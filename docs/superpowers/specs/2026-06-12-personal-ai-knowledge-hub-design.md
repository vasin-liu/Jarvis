...
以下资产用于沟通视觉与远期架构方向，**不替代本文档**。若与本文档或已实现代码冲突，以本文档与代码为准。

| 资产 | 路径 | 说明 |
| --- | --- | --- |
| UI 示意图 | `docs/personal_agent_ui_mockup.html` | 侧边栏 + 文档区 + Agent 对话面板的布局与视觉参考；在浏览器中打开预览 |
| 架构图 | `docs/personal_ai_agent_architecture.svg` | 个人工作智能体**远期全栈**架构（含知识图谱、Cursor 智能体等）；第一版仅实现其中「本地知识中枢」子集 |
...
**远期架构参考（非规范）：** `docs/personal_ai_agent_architecture.svg` — 展示完整愿景（含知识图谱、Cursor 智能体等）；下图仅为**第一版本地知识中枢**的实现架构。
...
**UI 布局参考（非规范）：** `docs/personal_agent_ui_mockup.html` — 三栏式（导航 / 内容 / Agent）与组件密度可作为实现时的视觉参考，具体交互以本节三个主视图为准。
